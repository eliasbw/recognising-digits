    ...
    def draw(self):
        ...
        if self.master is not None:
            ...
            self.master.create_rectangle(
                x_min, y_min, x_max, y_max, fill=_fill, outline=outline
            )
...
class CellGrid(Canvas):
    def __init__(self, master, nr_rows, nr_cols, cell_size, *args, **kwargs):
        ...
        Canvas.__init__(
            self,
            master,
            width=cell_size * nr_cols,
            height=cell_size * nr_rows,
            *args,
            **kwargs
        )
        self.height = nr_cols
    ...
    def return_grid_values(self):
        ...
        values = np.array(
            [
                np.array([int(cell.fill) for cell in col], dtype=float)
...
def create_view_input():
    ...
    evaluate = Button(
        frame, text="Evaluate", fg="black", command=lambda: grid.return_grid_values()
    )
